...
def train(config: dict, results_dir: Path = Path("results")):
    """Train A2C across all noise levels and seeds, evaluate each, save JSON results."""
    ...
    for noise in noise_levels:
        for seed in seeds:
            current += 1
            logger.info(f"[{current}/{total}] Training A2C | noise={noise:.2f} | seed={seed}")

            set_seeds(seed)
            agent = A2CAgent(config=config, noise=noise, seed=seed, train_df=train_df)
            ...
            output_path = results_dir / f"a2c_{symbol}_noise{noise:.2f}_seed{seed}_{timestamp}.json"
...
def evaluate(config: dict, results_dir: Path = Path("results")):
    """Load saved A2C models and evaluate on test data."""
    symbol = config["symbol"].lower()
    processed_dir = Path(config["paths"]["processed_dir"])
    test_df = pd.read_csv(processed_dir / f"{symbol}_test.csv")
    ...
    for noise in noise_levels:
        for seed in seeds:
            filename = f"A2C-noise={noise:.2f}-seed_{seed}"
            ...
            set_seeds(seed)
            model = A2CAgent.load(model_path)
            metrics = BaseAgent.evaluate(model, test_df, config, noise=0.0)
    ...
    logger.info("A2C evaluation complete.")
