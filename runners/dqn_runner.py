...
def train(config: dict, results_dir: Path = Path("results")):
    """Train DQN across all noise levels and seeds, evaluate each, save JSON results."""
    ...
    for noise in noise_levels:
        for seed in seeds:
            current += 1
            logger.info(f"[{current}/{total}] Training DQN | noise={noise:.2f} | seed={seed}")

            set_seeds(seed)
            agent = DQNAgent(config=config, noise=noise, seed=seed, train_df=train_df)
            ...
            output_path = results_dir / f"dqn_{symbol}_noise{noise:.2f}_seed{seed}_{timestamp}.json"
...
def evaluate(config: dict, results_dir: Path = Path("results")):
    """Load saved DQN models and evaluate on test data."""
    symbol = config["symbol"].lower()
    processed_dir = Path(config["paths"]["processed_dir"])
    test_df = pd.read_csv(processed_dir / f"{symbol}_test.csv")
    ...
    for noise in noise_levels:
        for seed in seeds:
            filename = f"DQN-noise={noise:.2f}-seed_{seed}"
            ...
            set_seeds(seed)
            model = DQNAgent.load(model_path)
            metrics = BaseAgent.evaluate(model, test_df, config, noise=0.0)
